...
namespace RestaurantTouchPanel {
    public partial class NabeWindow : Window {
        public NabeWindow() {
            InitializeComponent();
            this.Loaded += NabeWindow_Loaded;
        }

        private void NabeWindow_Loaded(object sender, RoutedEventArgs e) {
            ...
            if (sender is Button button) {
                ...
                switch (button.Tag) {
                    case "1":
                        ...
                        break;
                    case "2":
                        productName = "鶏白湯鍋(二人前)";
                        productPrice = 1080;
                        break;
                    case "3":
                        productName = "豚キムチ鍋(二人前)";
                        productPrice = 1080;
                        break;
                }
        ...
        private void SubCategoryButton_Click(object sender, RoutedEventArgs e) {
            if (sender is Button button) {
                ...
                if (subCategory == "おすすめ") {
                    var osusumeWindow = new OsusumeWindow();
                    osusumeWindow.Show();
                    this.Close();
                } else if (subCategory == "鍋") {
                    var nabeWindow = new NabeWindow();
                    nabeWindow.Show();
                    this.Close();
                } else {
