...
namespace RestaurantTouchPanel {
    public partial class ShochuWindow : Window {
        private const int MaxOrderTypes = 7; // 注文の種類数の制限

        public ShochuWindow() {
            InitializeComponent();
            this.Loaded += ShochuWindow_Loaded;
        }

        private void ShochuWindow_Loaded(object sender, RoutedEventArgs e) {
            UpdateOrderList();
        }
        ...
        private void SubCategoryButton_Click(object sender, RoutedEventArgs e) {
            if (sender is Button button) {
                ...
                if (subCategory == "酎ハイ") {
                    ...
                    var sakeWindow = new SakeWindow(); // 仮のウィンドウ名
                    sakeWindow.Show();
                } else if (subCategory == "ボトル") {
                    var bottleWindow = new BottleWindow(); // 仮のウィンドウ名
                    bottleWindow.Show();
                } else if (subCategory == "ボトルセット") {
                    var bottleSetWindow = new BottleSetWindow(); // 仮のウィンドウ名
